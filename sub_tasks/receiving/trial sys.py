...
#     SELECT "DocEntry", "Order No.", odsc_date, odsc_time, "Customer Code", "Normal(Or)Repair Order", "Status", odsc_status, "Order Criteria Status", odsc_createdby, "Order Total Amount", "Outlet"
...
#     #Routes data
#     routes = pd.read_sql("""
#     select "Date", "Trip", "Westlands", "Karen", "Eastlands", "Thika Road", "Mombasa Road", "Rongai", "Upcountry" from mabawa_staging.source_rider_routes
#     """,con=engine)
...
#     rongai = rongai.sort_values(by = ['rongai_time_back_at_hq2','CorrectTrip'],ascending = [True,False])
#     rongai = rongai.drop_duplicates(subset = 'doc_no',keep = 'first')    
#     #######
#     # westlands = westlands[westlands['CorrectTrip']==1]
#     westlands = westlands.sort_values(by = ['westlands_time_back_at_hq2','CorrectTrip'],ascending = [True,False])
#     westlands = westlands.drop_duplicates(subset = 'doc_no',keep = 'first')
#     #####
#     # mombasa = mombasa[mombasa['CorrectTrip']==1]
#     mombasa = mombasa.sort_values(by = ['mombasa_time_back_at_hq2','CorrectTrip'],ascending = [True,False])
#     mombasa = mombasa.drop_duplicates(subset = 'doc_no',keep = 'first')
#     ######
#     # eastlands = eastlands[eastlands['CorrectTrip']==1]
#     eastlands = eastlands.sort_values(by = ['eastlands_time_back_at_hq2','CorrectTrip'],ascending = [True,False])
#     eastlands = eastlands.drop_duplicates(subset = 'doc_no',keep = 'first')
#     #######
#     # karen = karen[karen['CorrectTrip']==1]
#     karen = karen.sort_values(by = ['karen_time_back_at_hq2','CorrectTrip'],ascending = [True,False])
#     karen = karen.drop_duplicates(subset = 'doc_no',keep = 'first')
#     ######
#     # thika = thika[thika['CorrectTrip']==1]
#     thika = thika.sort_values(by = ['thika_time_back_at_hq2','CorrectTrip'],ascending = [True,False])
#     thika = thika.drop_duplicates(subset = 'doc_no',keep = 'first')
...
#     rongai['receiving_time1'] = rongai.apply(lambda row: row['DateTime'] if row['CorrectTrip'] == 0 else row['receiving_time'],axis = 1)
#     westlands.rename(columns={'westlands_time':'receiving_time'}, inplace=True)
#     westlands['receiving_time1'] = westlands.apply(lambda row: row['DateTime'] if row['CorrectTrip'] == 0 else row['receiving_time'],axis = 1)
#     mombasa.rename(columns={'mombasa_time':'receiving_time'}, inplace=True)
#     mombasa['receiving_time1'] = mombasa.apply(lambda row: row['DateTime'] if row['CorrectTrip'] == 0 else row['receiving_time'],axis = 1)
#     eastlands.rename(columns={'eastlands_time':'receiving_time'}, inplace=True)
#     eastlands['receiving_time1'] = eastlands.apply(lambda row: row['DateTime'] if row['CorrectTrip'] == 0 else row['receiving_time'],axis = 1)
#     karen.rename(columns={'karen_time':'receiving_time'}, inplace=True)
#     karen['receiving_time1'] = karen.apply(lambda row: row['DateTime'] if row['CorrectTrip'] == 0 else row['receiving_time'],axis = 1)
#     upcountry.rename(columns={'upcountry_time':'receiving_time'}, inplace=True)
#     upcountry['receiving_time1'] = upcountry.apply(lambda row: row['DateTime'] if row['CorrectTrip'] == 0 else row['receiving_time'],axis = 1)
#     thika.rename(columns={'thika_time':'receiving_time'}, inplace=True)
#     thika['receiving_time1'] = thika.apply(lambda row: row['DateTime'] if row['CorrectTrip'] == 0 else row['receiving_time'],axis = 1)
...
#     receiving = pd.read_sql("""
#     select doc_entry, odsc_date, odsc_time, odsc_status, odsc_createdby, doc_no, cust_code, ods_outlet, "DateTime", trip_date, receiving_time, order_criteria_status, region from mabawa_dw.dim_receiving_data
#     """,con=engine)
...
#     receiving['cutoff'] = 15 # np.where(receiving['region']=='Upcountry',30,15)
...
#     select doc_entry, odsc_date, odsc_time, odsc_status, odsc_createdby, doc_no, cust_code, ods_outlet, "DateTime", trip_date, receiving_time, order_criteria_status, region, cutoff, duration, "hour" from
