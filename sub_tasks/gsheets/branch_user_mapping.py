...
def fetch_branch_user_mappings():
    ...
    query = """truncate mabawa_staging.source_exec_users;"""
    query = pg_execute(query)

    exec.to_sql('source_exec_users', con = engine, schema='mabawa_staging', if_exists = 'append', index=False)
    ...
    query = """truncate mabawa_staging.source_branch_user_mapping;"""
    query = pg_execute(query)

    df.to_sql('source_branch_user_mapping', con = engine, schema='mabawa_staging', if_exists = 'append', index=False)

    query1 = """
    update mabawa_staging.source_branch_user_mapping
    set branch_code = 'NAN'
...
def create_dim_branch_user_mapping():
    ...
    FROM mabawa_mviews.v_branch_user_mapping
    ...
    query = """truncate mabawa_dw.branch_user_mapping;"""
    query = pg_execute(query)
