...
with DAG(
    ...
    with TaskGroup('report') as report:
        with TaskGroup('build') as build:
            from rwanda.automations.queue_time.report import (
               build_rwanda_queue_time
            )

            build_rwanda_queue_time = PythonOperator(
                task_id='build_rwanda_queue_time',
                python_callable=build_rwanda_queue_time,
