...
def create_rejection_report(
    ...
    unique_insurance_merge["Date"] = pd.to_datetime(
        unique_insurance_merge["Date"], 
        dayfirst=True
    )
    unique_insurance_merge["CreateDate"] = pd.to_datetime(
        unique_insurance_merge["CreateDate"],
        dayfirst=True
    )
    ...
    if selection == "Daily":
        daily_insurance_orders = unique_insurance_merge[
            unique_insurance_merge["CreateDate"].dt.date == start_date]
        ...
        daily_unique_rejections = daily_rejections.drop_duplicates(
            subset=["Order Number", "Date", "Time"])
        ...
        daily_rej_total = pd.merge(
            daily_rej_total, daily_conversion_pivot, on="Outlet", how="left").fillna(0)
        ...
        with pd.ExcelWriter(f"{path}draft_upload/rejections_report.xlsx") as writer:
            ...
            rejections_daily_data.to_excel(
                writer, 
    ...
    if selection == "Weekly":
        ...
        weekly_insurance_orders["Week Range"] = weekly_insurance_orders.apply(
            lambda row: check_date_range(row, "Date"), axis=1)
        weekly_insurance_orders = weekly_insurance_orders[
            weekly_insurance_orders["Week Range"] != "None"]
        ...
        unique_weekly_rejections = weekly_rejections.drop_duplicates(
            subset=["Date", "Order Number", "Time"])

        weekly_rejection_conversion = unique_weekly_rejections[
            unique_weekly_rejections["Order Number"].isin(
        ...
        weekly_rejections_pivot = pd.pivot_table(
            unique_weekly_rejections,
            index="Outlet",
            columns="Week Range",
            values="Order Number",
            aggfunc="count"
        ...
        weekly_rejection_branches = pd.merge(
            branch_data[["Outlet", "RM", "SRM"]],
        ...
        final_reje_merge = final_reje_merge.swaplevel(
            0, 1, axis=1).sort_index(axis=1, level=0)
        final_reje_merge = final_reje_merge.reindex(
            ["Total Orders", "Rejected", "Converted"], axis=1, level=1)
        ...
        for date in final_reje_merge.columns.levels[0]:
            ...
            final_reje_merge.insert(
                final_reje_merge.columns.get_loc((date, 'Rejected')) + 1, col_name, (final_reje_merge[(date, 'Rejected')] / final_reje_merge[(
                    date, 'Total Orders')] * 100).replace([np.inf, -np.inf], np.nan).fillna(0).round().astype(int)
            )

        for date in final_reje_merge.columns.levels[0]:
            col_name = (date, '%Conversion')
            result = (final_reje_merge[(date, 'Converted')] / final_reje_merge[(date, 'Rejected')]
                      * 100).round(0).replace([np.inf, -np.inf], np.nan).fillna(0).astype(int)
            final_reje_merge.insert(final_reje_merge.columns.get_loc(
                (date, 'Converted')) + 1, col_name, result)
        ...
        with pd.ExcelWriter(f"{path}draft_upload/rejections_report.xlsx") as writer:
            ...
            weekly_rejections_data.to_excel(
                writer, 
                sheet_name="weekly_rejections_data", 
                index=False
            )
            ewc_weekly_pivot.to_excel(
                writer, 
                sheet_name="ewc_summary", 
                index=False
            )
            branches_rejections_weekly.to_excel(
                writer, 
    ...
    if selection == "Monthly":
        ...
        final_monthly_rejections = final_monthly_rejections.reindex(
            ["Total Orders", "Rejected", "%Rejected"], level=1, axis=1)
        final_monthly_rejections = get_rm_srm_total_multiindex(
            final_monthly_rejections, week_month="Month")

        new_cols = [(y, x) if x in ['Outlet', 'RM', 'SRM'] else (x, y)
                    for x, y in final_monthly_rejections.columns]
