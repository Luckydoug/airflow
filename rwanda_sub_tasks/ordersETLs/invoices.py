...
def fetch_sap_invoices():
    ...
    if invoices_df.empty:
        ...
        invoices_details['invoice_total_amount'] = pd.to_numeric(invoices_details['invoice_total_amount'])
        invoices_details['doctype'] = invoices_details['doctype'].astype(str)

        print('TRANSFORMATION! Adding new invoice header rows')
        invoices_details.to_sql('source_invoices', con = engine, schema='voler_staging', if_exists = 'append', index=False)
        ...
        itemdetails_df.to_sql('source_invoices_details', con = engine, schema='voler_staging', if_exists = 'append', index=False)