...
def fetch_sap_items():
    ...
    for i in range(1, pages+1):
        page = i
        url = f"https://10.40.16.9:4300/RWANDA_BI/XSJS/BI_API.xsjs?pageType=GetItemDetails&pageNo={page}&FromDate={FromDate}&ToDate={ToDate}&SessionId={SessionId}"
    ...
    # itemsdf.to_sql('source_items', con = engine, schema='voler_staging', if_exists = 'append', index=False)
...
def fetch_item_groups():
    ...
    for i in range(1, pages+1):
        ...
        itemsgroup = requests.request("GET", url, headers=headers, data=payload, verify=False)
    ...
    itemgroupdf.rename (
        columns = {
            'ItmsGrpCod':'item_grp_code',
            'ItmsGrpNam':'item_grp_name'}
        ,inplace=True)
