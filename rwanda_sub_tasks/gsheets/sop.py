...
def fetch_sop_branch_info():
    ...
    query = """truncate voler_staging.sop_branch_info;"""
    query = pg_execute(query)

    sh[sh['Country']=='RW'][['branch','branch_code','srm','rm']].to_sql('sop_branch_info', con=engine, schema='voler_staging', if_exists = 'append', index=False)
...
def fetch_sop():
    ...
    query = """truncate voler_staging.source_sop;"""
    query = pg_execute(query)

    df['sop_date'] = pd.to_datetime(df['sop_date'], dayfirst=True, errors='coerce')

    df[cols].to_sql('source_sop', con=engine, schema='voler_staging', if_exists = 'append', index=False)

# fetch_sop()
# fetch_sop_branch_info()
