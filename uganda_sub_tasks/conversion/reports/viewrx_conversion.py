...
def create_views_conversion(data):
    ...
    with pd.ExcelWriter(f"{uganda_path}conversion/viewrx/overall.xlsx") as writer:
    ...
    with pd.ExcelWriter(f"{uganda_path}conversion/viewrx/sales_persons.xlsx") as writer:
        for group, dataframe in ewc_conversion.groupby("Branch"):
            name = f'{group}'
            dataframe.to_excel(writer, sheet_name=name, index=False)

    with pd.ExcelWriter(f"{uganda_path}conversion/viewrx/branches.xlsx") as writer:
        for group, dataframe in branch_conversion.groupby("Branch"):
            name = f'{group}'
            dataframe.to_excel(writer, sheet_name=name, index=False)

    with pd.ExcelWriter(f"{uganda_path}conversion/viewrx/non_conversions.xlsx") as writer:
