...
# api details
pagecount_url = f"https://10.40.16.9:4300/UGANDA_BI/XSJS/BI_API.xsjs?pageType=GetInvoiceDetails&pageNo=1&FromDate={FromDate}&ToDate={ToDate}&SessionId={SessionId}"
pagecount_payload={}
pagecount_headers = {}
...
def fetch_sap_invoices():
    ...
    for i in range(1, pages+1):
        page = i
        url = f"https://10.40.16.9:4300/UGANDA_BI/XSJS/BI_API.xsjs?pageType=GetInvoiceDetails&pageNo={page}&FromDate={FromDate}&ToDate={ToDate}&SessionId={SessionId}"
    ...
    if invoices_df.empty:
        ...
        invoices_details = invoices_details.drop(['doctype'],axis=1)
        invoices_details.to_sql('source_invoices', con = engine, schema='mawingu_staging', if_exists = 'append', index=False)

        print('TRANSFORMATION! Added new invoice header rows_again')
        print(invoices_details)
        '''
        ...
        itemdetails_df.to_sql('source_invoices_details', con = engine, schema='mawingu_staging', if_exists = 'append', index=False)
