...
def fetch_sap_orderscreendetails():
    ...
    for i in range(1, pages+1):
        page = i
        print(i)
        url = f"https://10.40.16.9:4300/UGANDA_BI/XSJS/BI_API.xsjs?pageType=GetOrderDetails&pageNo={page}&FromDate={FromDate}&ToDate={ToDate}&SessionId={SessionId}"
    ...
    query = """truncate table mawingu_staging.landing_orderscreen;"""
    query = pg_execute(query)

    print("Dropped Landing Table")

    orderscreendf.columns = orderscreendf.columns.str.replace('[#,@,&,%,(,)]', '_')
    orderscreendf.to_sql('landing_orderscreen', con=engine, schema='mawingu_staging', if_exists = 'append', index=False)  
...
def update_to_source_orderscreen():
    ...
    (SELECT 
        ...
        *
        FROM mawingu_staging.landing_orderscreen) as t
    where t.r = 1;
    """, con=engine)
