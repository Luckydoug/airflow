...
pagecount_url = f"https://10.40.16.9:4300/UGANDA_BI/XSJS/BI_API.xsjs?pageType=GetOrders&pageNo=1&FromDate={FromDate}&ToDate={ToDate}&SessionId={SessionId}"
pagecount_payload={}
pagecount_headers = {}
...
def fetch_sap_orders():
    ...
    for i in range(1, pages+1):
        page = i
        print(page)
        url = f"https://10.40.16.9:4300/UGANDA_BI/XSJS/BI_API.xsjs?pageType=GetOrders&pageNo={page}&FromDate={FromDate}&ToDate={ToDate}&SessionId={SessionId}"
    ...
    #orders_header.to_sql('source_tests', con = engine, schema='mawingu_staging', if_exists = 'append', index=False)
    ...
    """
    query = """truncate mawingu_staging.landing_source_orders_line;"""
    query = pg_execute(query)

    orders_line = orders_line.drop_duplicates(['id', 'doc_internal_id', 'item_no'])

    orders_line.to_sql('landing_source_orders_line', con = engine, schema='mawingu_staging', if_exists = 'append', index=False)
...
def update_source_orders_line():
    ...
    SELECT 	distinct
        ...
        flag_for_order
    FROM mawingu_staging.landing_source_orders_line;
    """, con=engine)
