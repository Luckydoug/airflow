...
def fetch_sunglasses_targets():
    ...
    drop_table = """truncate table mawingu_dw.sunglasses_targets;"""
    drop_table = pg_execute(drop_table)
    
    sh[sh['country']=='UG'][['branch','target']].to_sql('sunglasses_targets', con = engine, schema='mawingu_dw', if_exists = 'append', index=False)

    print('sunglasses targets pulled')
