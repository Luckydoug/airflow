...
def fetch_branch_user_mappings():
    ...
    query = """truncate mawingu_staging.source_exec_users;"""
    query = pg_execute(query)

    exec.to_sql('source_exec_users', con = engine, schema='mawingu_staging', if_exists = 'append', index=False)

    df = sh.worksheet_by_title('Branch Mapping -  UG')
    ...
    query = """truncate mawingu_staging.source_branch_user_mapping;"""
    query = pg_execute(query)

    df.to_sql('source_branch_user_mapping', con = engine, schema='mawingu_staging', if_exists = 'append', index=False)

    query1 = """
    update mawingu_staging.source_branch_user_mapping
    set branch_code = 'NAN'
...
def create_dim_branch_user_mapping():
    ...
    FROM mawingu_mviews.v_branch_user_mapping
    ...
    query = """truncate mawingu_dw.branch_user_mapping;"""
    query = pg_execute(query)
     
    data.to_sql('branch_user_mapping', con = engine, schema='mawingu_dw', if_exists = 'append', index=False)
